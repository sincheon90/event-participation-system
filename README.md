...
イベント開始直後のアクセス集中を想定し、イベント参加 API に対して通常負荷、高負荷、スパイク、継続負荷、重複参加の 5 シナリオを JMeter で実行しました。

テスト環境は API 1 台、Kafka Consumer Worker 3 台、Kafka Topic 3 Partition で構成し、2026-07-15 に API 応答性能と非同期結果保存時間を同一テスト実行から測定しました。

### 結果サマリー

| シナリオ | リクエスト数 | HTTP 結果 | API p99 | Throughput | 非同期処理 p99 |
|---|---:|---|---:|---:|---:|
| 通常負荷 | 10,000 | `201` 10,000 件 | 45 ms | 337.06 req/s | 1.27 秒 |
| 高負荷 | 50,000 | `201` 50,000 件 | 638 ms | 1,560.16 req/s | 20.51 秒 |
| スパイク | 10,000 | `201` 10,000 件 | 897.98 ms | 1,458.36 req/s | 4.80 秒 |
| 重複参加 | 1,000 | `201` 1 件 / `409` 999 件 | 870.94 ms | 1,112.35 req/s | 1.03 秒 |
| 継続負荷 | 100,000 | `201` 100,000 件 | 59 ms | 1,860.78 req/s | 32.16 秒 |

### 確認できたこと

- 全 171,000 リクエストでタイムアウトおよび 5xx は発生しませんでした。
- 同一ユーザーによる 1,000 件の同時参加要求は、成功 1 件と `409 Conflict` 999 件に制御されました。
- API リクエスト数と非同期結果ログの件数・status 分布が一致し、171,000 件すべての結果ログ保存を確認しました。
- API は継続負荷で 1,860.78 req/s、p99 59 ms を記録しました。
- 一方、非同期処理は高負荷で p99 20.51 秒、継続負荷で p99 32.16 秒となり、Consumer 側の処理能力改善が次の課題です。

### 関連ドキュメント

- [負荷テスト計画](docs/load-test-plan.md)
- [負荷テスト結果](docs/load-test-result.md)
- [JMeter Test Plan](load-test/participation-load-test.jmx)
- [Git 管理対象の集計データ](load-test/results)

Raw JTL と JMeter HTML Dashboard は生成ファイル数と容量が大きいため Git には含めず、API・非同期処理の集計 CSV と各 Dashboard の `statistics.json` のみを管理対象としています。
